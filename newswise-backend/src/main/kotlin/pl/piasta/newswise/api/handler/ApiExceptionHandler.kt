...
class ApiExceptionHandler : WebExceptionHandler, Ordered {
    override fun handle(exchange: ServerWebExchange, ex: Throwable): Mono<Void> = apiExceptions[ex::class]?.let {
        ResponseStatusException(it, ex.rootCause.message, ex).toMono()
    } ?: ex.toMono()
