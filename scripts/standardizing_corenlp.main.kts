...
val standarizedDataPath: Path = dataPath.resolve("data_corenlp.dat")
...
fun standarizedDataFlow() = originalDataFlow().parallelMap { it.halveAndJoinReversed("\t") }
...
runBlocking(IO) {
    println("Standardizing for CoreNLP started.")
    runCatching {
        standarize()
    }.onSuccess {
