...
# Extracting labels and F1 scores from each fold
fold_lines = data.split("### Fold")[1:]

# Initialize dictionaries to store aggregated F1 scores for each category
all_f1_scores = {label: [] for label in
                 ["Business&Money", "Travel&Food", "World&Politics", "Entertainment&Arts", "SciTech&Education",
                  "Crime&Legal", "Sports&Health", "Lifestyle", "Society&Religion"]}
...
for fold in fold_lines:
    ...
    data_lines = [line for line in lines if line.startswith("INFO Cls")]
    ...
    for line in data_lines:
        ...
        all_f1_scores[label].append(f1_score)

# Calculate the mean F1 score for each category
mean_f1_scores = {label: sum(scores) / len(scores) for label, scores in all_f1_scores.items()}

# Plotting the bar chart with mean F1 score values
plt.figure(figsize=(10, 6))
bars = plt.barh(list(mean_f1_scores.keys()), list(mean_f1_scores.values()), color='skyblue')
...
# Add mean F1 score values next to each bar
for bar, value in zip(bars, mean_f1_scores.values()):
